...
/*function optionsdemo_load_textdomain(){
	load_plugin_textdomain('optionsdemo', false, plugin_dir_path( __FILE__ )."/languages");
}
add_action('plugins_loaded','optionsdemo_load_textdomain');*/
...
class OptionsDemo_Settings_Page {
	public function __construct() {
		...
		add_action('plugin_action_links_'.plugin_basename(__FILE__), array($this, 'optionsdemo_settings_link'));
	}

	public function optionsdemo_settings_link($links){
		$newlink = sprintf("<a href='%s'>%s</a>",'options-general.php?page=optiondemo',__('Settings','optionsdemo'));
		$links[] = $newlink;
		return $links;
	}
	...
	public function optionsdemo_setup_sections() {
		add_settings_section('optionsdemo_section',__('Demonstration of plugin settings page'),array(),'optionsdemo');
	}
	...
	public function optionsdemo_setup_fields() {
		$fields = array(
			...
			array(
				'label' => __('Longitute','optionsdemo'),
				'id' => 'optionsdemo_longitute',
				'type' => 'text', 
				'section' => 'optionsdemo_section',
				'placeholder' => __('Longitute','optionsdemo'),
			),
			array(
				'label' => __('Zoom Level','optionsdemo'),
				'id' => 'optionsdemo_zoomlavel',
				'type' => 'text', 
				'section' => 'optionsdemo_section',
			),
			array(
			...
			array(
				'label' => __('Something','optionsdemo'),
				'id' => 'optionsdemo_something',
				'type' => 'text', 
				'section' => 'optionsdemo_section',
			),
			array(
	...
	public function optionsdemo_field_callback($field){
		...
		switch ($field['type']) {
			case 'textarea':
				printf('<textarea name="%1$s" id="%1$s" placeholder="%2$s" rows="10" cols="50" >',
					$field['id'],
				...
				break;
			default:
				printf('<input name="%1$s" id="%1$s" type="%2$s" placeholder="%3$s" value="%4$s" cols="50" >',
					$field['id'],
					$field['tupe'],
					isset($field['placeholder'])?$field['placeholder']:'',
					$value
